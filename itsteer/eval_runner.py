...
from .prompts import build_prompt, CLASS_LABELS
from .utils import get_device
from .eval_metrics import accuracy, content_effect_metrics

def score_candidates(model, tokenizer, prompt_text: str, candidates: list[str] = CLASS_LABELS, max_length: int = 2048) -> list[float]:
    ...
    with torch.no_grad():
        ...
        # (B, L-1)
        per_pos_loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1)).view(shift_labels.size())
...
def batch_predict(model, tokenizer, syllogisms: list[str]) -> list[bool]:
    preds = []
    for s in syllogisms:
        prompt = build_prompt(s)
        logps = score_candidates(model, tokenizer, prompt.text, candidates=prompt.labels)
        pred = True if logps[0] > logps[1] else False  # VALID vs INVALID
        preds.append(pred)
    return preds

def evaluate_model(model_name: str, dataset, vector_conf: Optional[dict] = None) -> dict:
    device = get_device()
    ...
    if vector_conf is not None:
        steerer = Steerer(model, layer_idx=vector_conf['layer'], v=vector_conf['v'], alpha=vector_conf['alpha'], last_n=vector_conf.get('last_n', 32))
        ctx = steerer.context()
    else:
        ctx = None
    ...
    try:
        ...
        preds = batch_predict(model, tok, [ex.syllogism for ex in dataset])
        acc = accuracy(y_true, preds)
        ce = content_effect_metrics(y_true, preds, plaus)
        return {'accuracy': acc, **ce}
    finally:
        if ctx is not None:
